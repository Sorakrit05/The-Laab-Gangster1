...
struct Player {
    ...
    unordered_set<int> lastTurnIngredientIndexes; // เก็บส่วนผสมที่เลือกในเทิร์นที่แล้ว
...
bool addIngredient(Player& player, const string& color, const vector<string>& availableIngredients, int index) {
    if (player.lastTurnIngredientIndexes.find(index) != player.lastTurnIngredientIndexes.end()) {
        cout << "You cannot select the same ingredient as last round. Choose another.\n";
        return false;
    }
    
    player.lastTurnIngredientIndexes.insert(index);
    player.ingredients.push_back(availableIngredients[index]);
...
void inputIngredientsByColor(Player& player, const vector<string>& colors, unordered_map<string, unordered_set<string>>& previousRoundSelections) {
    ...
    player.lastTurnIngredientIndexes.clear();
    ...
    for (string color : uniqueColors) {
        ...
        for (size_t i = 0; i < availableIngredients.size(); ++i) {
            cout << i + 1 << ". " << availableIngredients[i] << endl;
        }
        ...
        while (!valid) {
            ...
            cin >> index;
            index--; // ปรับให้เป็น index ที่เริ่มจาก 0

            if (cin.fail() || index < 0 || index >= availableIngredients.size()) {
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
            ...
            valid = addIngredient(player, color, availableIngredients, index);
        }
    }
}
