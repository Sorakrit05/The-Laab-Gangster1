...
void inputIngredientsByColor(Player& player, const vector<string>& colors) {
    ...
    for (const string& color : uniqueColors) {
        ...
        for (size_t i = 0; i < remainingIngredients.size(); ++i) {
            cout << i + 1 << ". " << remainingIngredients[i] << "\n";
        }
        ...
        while (!valid) {
            cout << "Select ingredient number (" << 1 << " - " << remainingIngredients.size() << "): ";
            int index;
            cin >> index;

            if (cin.fail() || index < 1 || index > remainingIngredients.size()) {
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                cout << "Invalid input. Please enter a number between 1 and " << remainingIngredients.size() << ".\n";
                continue;
            }

            ingredient = remainingIngredients[index - 1];

            // หาดัชนีของวัตถุดิบที่เลือกในรายการทั้งหมด
            auto it = find(availableIngredients.begin(), availableIngredients.end(), ingredient);
            if (it != availableIngredients.end()) {
                int selectedIndex = distance(availableIngredients.begin(), it) + 1;
                selectedIndicesPerColor[color].insert(selectedIndex); // บันทึกดัชนีที่เลือกไปแล้ว
