...
Player handleCardEffect(Player player, const Card& card) {
    auto distributeExcessIngredient = [&](int& ingredient, const std::vector<int>& limits) {
        if (ingredient > limits.size()) {
            ingredient--;
    ...
    if (card.name == "Suan Krua") {
        player.herbs++;
        distributeExcessIngredient(player.herbs, player.recipe.herbs);
    }
    else if (card.name == "Moitgure") {
        player.herbs = std::max(0, player.herbs - 1);
    }
    else if (card.name == "Regular Customer") {
        player.mainIngredients++;
        distributeExcessIngredient(player.mainIngredients, player.recipe.mainIngredients);
    }
    else if (card.name == "Moldy Cutting Board") {
        player.mainIngredients = std::max(0, player.mainIngredients - 1);
    }
    else if (card.name == "The Kitchen") {
        player.spices++;
        distributeExcessIngredient(player.spices, player.recipe.spices);
    }
    else if (card.name == "Rainy Season") {
        player.specialIngredients++;
        distributeExcessIngredient(player.specialIngredients, player.recipe.specialIngredients);
    }
    else if (card.name == "Falling Mortar") {
        player.spices = std::max(0, player.spices - 1);
    }
    else if (card.name == "Falling Tray") {
        player.specialIngredients = std::max(0, player.specialIngredients - 1);
    }
    return player;
}
...
void inputIngredientsByColor(Player& player, const vector<string>& colors) {
    ...
    unordered_map<string, unordered_set<int>> selectedIndicesPerColor;
    ...
    while (true) {
        ...
        for (const auto& color : uniqueColors) {
            if (*ingredientCountMap[color] < ingredientMap[color].size()) {
                availableColors.push_back(color);
            }
        }

        if (availableColors.empty()) {
            cout << "No more ingredients can be added. All categories are full.\n";
            return;
        }

        cout << "\nSelect a color to add an ingredient from:\n";
        for (size_t i = 0; i < availableColors.size(); ++i) {
            cout << i + 1 << ". " << availableColors[i] << endl;
        }

        int colorChoice;
        cout << "Enter your choice (1-" << availableColors.size() << "): ";
        cin >> colorChoice;
        
        if (cin.fail() || colorChoice < 1 || colorChoice > availableColors.size()) {
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
            cout << "Invalid input. Please select a valid color.\n";
            continue;
        }

        string selectedColor = availableColors[colorChoice - 1];
        vector<string>& availableIngredients = ingredientMap[selectedColor];
        unordered_map<int, int> indexMapping;
        vector<string> filteredIngredients;
        int displayIndex = 1;
        for (size_t i = 0; i < availableIngredients.size(); ++i) {
            if (selectedIndicesPerColor[selectedColor].find(i + 1) == selectedIndicesPerColor[selectedColor].end()) {
        ...
        if (filteredIngredients.empty()) {
            cout << "No available ingredients for " << selectedColor << ". Try another color.\n";
            continue;
        }

        cout << "\nAvailable ingredients for " << selectedColor << ":\n";
        ...
        if (cin.fail() || ingredientIndex < 1 || ingredientIndex > filteredIngredients.size()) {
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
        ...
        int originalIndex = indexMapping[ingredientIndex] + 1;
        string ingredient = availableIngredients[originalIndex - 1];
        
        if (selectedIngredients.find(ingredient) != selectedIngredients.end()) {
            cout << "You have already selected this ingredient before. Please choose another one.\n";
            continue;
        }
        
        (*ingredientCountMap[selectedColor])++;
        selectedIndicesPerColor[selectedColor].insert(originalIndex);
        selectedIngredients.insert(ingredient);
        player.ingredients.push_back(ingredient);
        cout << "Selected " << ingredient << " for color " << selectedColor << ".\n";
        break;
    }
}
